...
`retry_oauth.py` 会读取 `registered_accounts.txt` 中的账号状态，默认重试所有非 `oauth=ok` 的账号；成功后会把对应行改成 `oauth=ok`。

```bash
# 默认读取 config.json 中的 output_file，只重试非 oauth=ok 的账号
python3 retry_oauth.py

# 只重试 oauth=failed 的账号
python3 retry_oauth.py --mode failed

# 指定账号文件，并把 token 输出到独立目录
python3 retry_oauth.py --accounts-file ./registered_accounts.txt --output-dir ./oauth_retry_output

# 不覆盖原账号文件，把更新后的状态写到新文件
python3 retry_oauth.py --status-output ./registered_accounts.updated.txt
```

主要参数：

- `--accounts-file`：账号状态文件路径，默认使用 `config.json` 里的 `output_file`
- `--status-output`：更新后的账号状态输出路径，默认覆盖原文件
- `--output-dir`：OAuth 成功后的输出目录，会生成 `ak.txt`、`rk.txt` 和 `tokens/`
